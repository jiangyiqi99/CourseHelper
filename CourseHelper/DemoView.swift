...
struct DemoLoginView: View {
    ...
    private var loginForm: some View {
        Form {
            Section(header: Text("\(universityName) 登录")) {
    ...
    private func handleLogin() {
        if username == "test" && password == "123456" {
            self.capturedData = Self.demoData
            showConfirmButton = true
            chosenSchool = "演示模式"
            isLoggedIn = true
            alertTitle = "成功"
            alertMessage = "成功获取到课表！\n请返回主界面导入课表"
            showAlert = true
        } else {
            alertTitle = "错误"
            alertMessage = "演示模式登录错误"
            showAlert = true
        }
    }

    // MARK: - Demo Data
    private static let demoData = """
        var fakeCourses = [];
...
class DemoDecode {
    ...
    func MainProcess(capturedString: String, reminderTime: String, onAlert: ((String, String) -> Void)? = nil){
